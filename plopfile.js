...
module.exports = function (plop) {
  ...
  plop.setHelper(
    'capitalize',
    (text) => pluralize.singular(text).charAt(0).toUpperCase() + pluralize.singular(text).slice(1),
  );
  ...
  plop.setHelper(
    'capitalizePlural',
    (text) => pluralize(text).charAt(0).toUpperCase() + pluralize(text).slice(1),
  );
  ...
  // New timestamp helper in YYYYMMDDHHmmss format
  plop.setHelper('migrationTimestamp', () => dayjs().format('YYYYMMDDHHmmss'));
  ...
  plop.setGenerator('service', {
    ...
    actions: [
      ...
      {
        type: 'add',
        path: 'src/migrations/{{migrationTimestamp}}-create-{{plural service}}.js',
