  ...
  if (queries.search) {
    ...
    if (projection) {
      ...
      pagination.isLastPage = totalResult =
        page !== 0 && page + 1 === pages ? true : totalResult <= limit ? true : false;
...
export const getAllQuery2 = async <T extends Model>({
  ...
  if (search) {
    ...
    const { rows: data, count: totalResult } = (await model.findAndCountAll(_query)) as {
      rows: T[];
      count: number;
    };
    result.data = data;
    ...
    result.pagination.isLastPage =
      page !== 0 && page + 1 === pages ? true : totalResult <= _query.limit ? true : false;
...
export const getAllQuery = async <T extends Model>({
  ...
  populate,
  searchColumns,
}: IQueryData & { searchColumns: string[] }): Promise<{ data: T[]; pagination: IPagination }> => {
