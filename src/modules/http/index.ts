...
export class Http {
    ...
    try {
      ...
      this.axiosInstance.interceptors.response.use(
        ...
        async (error: AxiosError) => {
          const shouldRetry = this.retryEnabled && error.config && this.isRetryable(error);
  ...
  private logResponse(request: AxiosRequestConfig = {}, response: AxiosResponse, service: string) {
  ...
  public async patch(
    ...
    try {
      const response = await this.axiosInstance.patch(url, options.body || {}, options);
      return this.logResponse(options, response, service);
    } catch (err) {
